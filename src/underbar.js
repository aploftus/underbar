(function() {
  ...
  _.zip = function() {
    ...
    var results = [];
    var subArr;
    var len = _.reduce(args, function(acc, arg) {
      return arg.length > acc ? arg.length : acc;
    }, 0);
  ...
  _.flatten = function(nestedArray, result) {
    return _.reduce(nestedArray, function(acc, item) {
      if (Array.isArray(item)) {
        return acc.concat(_.flatten(item));
      }
  ...
  _.intersection = function() {
    var itemArr = arguments[0];
    var searchArrs = [...arguments].slice(1);

    return _.filter(itemArr, function(item) {
      return _.every(searchArrs, function(arr) {
  ...
  _.difference = function(array) {
    ...
    return _.filter(array, function(item) {
      return _.every(searchArrs, function(searchArr) {
        return !_.contains(searchArr, item);
      });
    });
  };
